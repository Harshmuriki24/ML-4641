...
As with existing research, we conclude that neural networks, decision-trees, and logistic regression remain effective ways to characterize student dropout data. K-Means clustering may not be well suited for analyzing dropout data due to the indistinct clusters and high complexity (non-convex shape). 'Tuition-Fees Up to Date' and 'Curricular units 2nd sem (grade)' are pertinent areas to focus on for university systems in improving student retention, and the impact of parental/guardian educational/professional backgrounds may not be as important as previously thought. Our methods have much room for improvement, especially regarding hyperparameter tuning and some accuracy rates below 80%. However, we believe our method contributes to the ongoing discussion of incomplete post-secondary education with it's combination of existing study methods with unsupervised learning and a large/unique dataset.
