...
Using the Elbow method, we can see that about 5 clusters is optimal. This number of clusters thus maximizes intracluster similarity and minimizes intercluster similarity.
<img src="images/elbow.png" width="800" height="500"/>

However, the neural networks model that was constructed using sklearn had an astounding 89% accuracy. The neural networks model demonstrated increased accuracy as the number of features increased (meaning all features were relevant and useful for the NN model), though the number of epochs did not significantly increase accuracy after 5 epochs (as higher epochs may lead to overfitting). This high accuracy rate can be attributed to the capacity of neural networks to handle complex data, such as student dropout data with the multitude of features.
...
Our study utilized the random forest decision-tree model, k-means clustering, logistic regression, and neural networks: most of which have been used in prior research. Using the logistic regression and random forest model, we identified 'Tuition-Fees Up to Date' and 'Curricular units 2nd sem (grade)' as potential areas of focus in improving the student retention rate. These results align with previous studies in the impact of socioeconomic status on outcomes in college, though we identified the amount of credit hours selected (and more importantly, the grade achieved in each course, especially in the second semester) were strong predictors of student dropout rate. However, it is worth mentioning that the logistic regression and decision-tree model had differing weights assigned to parental occupation and qualification, though in both models the feature importance was exceedingly low. These results suggest that more research and large datasets may be necessary to fully understand the myriad of features influencing retention rate.

As with existing research, we conclude that neural networks, decision-trees, and logistic regression remain effective ways to characterize student dropout data. K-Means clustering may not be well suited for analyzing dropout data due to the indistinct clusters and high complexity (non-convex shape). 'Tuition-Fees Up to Date' and 'Curricular units 2nd sem (grade)' are pertinent areas to focus on for university systems in improving student retention, and the impact of parental/guardian educational/professional backgrounds may not be as important as previously thought. Our methods have much room for improvement, especially regarding hyperparameter tuning and some accuracy rates below 80%. As well, our dataset and models may not be as generalizable as we would like, as the cultural and social impacts across countries may also impact student dropout rate. However, we believe our method contributes to the ongoing discussion of incomplete post-secondary education with it's combination of existing study methods with unsupervised learning and wide array of features present in the dataset.
